const CSSStyleExtractor = (() => {
  ...
  /**
   * Retrieves a `CSSRuleList` object containing CSS style rules for a specified selector
   * and copy them to a new CSSStyleSheet object.
   * @returns {CSSRuleList | null} - `CSSRuleList` object containing the CSS style rules: `CSSStyleRules` or null
   */
  function getCSSStyleRules() {
    ...
    let sheetCssRules = sheet.cssRules;
    // Create a new CSSStyleSheet object.
    let CSSRules = new CSSStyleSheet();
    let index = 0;
    for (let rule of sheetCssRules) {
      if (rule.constructor.name === "CSSStyleRule") {
        if (isSelectorIncluded(rule)) {
          //Make a copy of CSS rules that have a specified selector to a new CSSStyleSheet object.
          CSSRules.insertRule(rule.cssText, index);
          index++;
        }
      } else if (rule.constructor.name === "CSSMediaRule") {
        let parentRule = rule;

        // Check if CSSMediaRule has a specified selector
        if (isMediaRuleHasSelector(rule.cssRules)) {
          CSSRules.insertRule(parentRule.cssText, index);
          index++;
        }
  ...
  /**
   * Checks whether the provided selector exists within the given 'CSSMediaRule' rule.
   *
   * @param {CSSRules} mediaRules - The CSS rules object to search within.
   * @returns {Boolean} - `true` if the provided selector is present in the 'CSSMediaRule'.
   */
  function isMediaRuleHasSelector(mediaRules) {
    let mediaRuleHasSelector = false;
    for (let rule of mediaRules) {
      if (rule.constructor.name === "CSSMediaRule") {
        let currentMediaRuleHasSelector = isMediaRuleHasSelector(rule.cssRules);
        if (currentMediaRuleHasSelector) {
          return currentMediaRuleHasSelector;
        }
      } else if (rule.constructor.name === "CSSStyleRule") {
        if (isSelectorIncluded(rule)) {
          return true;
  ...
  function loadStyleSheet() {
    ...
    for (const sheet of styleSheets) {
      ...
      // for cases where property href is null e.g when ownerNode: style.
      if (hrefSheet !== null) {
    ...
    console.error(
      `There is no CSS file named ${cse_styleSheetName}. Please check if the CSS file has been appended to the <head> section.`
    );
    return null;
  }
